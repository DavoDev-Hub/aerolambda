...
import { generateBoardingPass } from '@/utils/ticketGenerator'; // ✅ IMPORTANTE: Usamos el generador compartido
...
export default function MyBookingsPage() {
  ...
  // ✅ NUEVA FUNCIÓN: Usa el generador compartido para el PDF
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleDownloadTicket = async (reservation: any) => {
    if (isDownloading) return;
    setIsDownloading(true);
    try {
      // Llamamos a la utilidad compartida
      await generateBoardingPass(reservation);
    } catch (error) {
      console.error(error);
      toast.error('No se pudo generar el boleto');
    } finally {
  ...
  const pasadosCount = reservations.filter(
    (r) => r.estado === 'completada' || (r.estado === 'confirmada' && r.vuelo && new Date(r.vuelo.fechaSalida) < new Date()),
  ).length;
      ...
      <div className="min-h-screen bg-slate-900">
        ...
        <div className="flex items-center justify-center h-[calc(100vh-64px)]">
          <div className="text-center">
            ...
            <p className="text-blue-200 font-medium">Cargando tus viajes...</p>
          </div>
        </div>
      </div>
    );
  if (error)
    return (
      <div className="min-h-screen bg-slate-900">
        <Header />
        <div className="flex items-center justify-center h-[calc(100vh-64px)]">
          <div className="text-center bg-white/10 backdrop-blur-md p-8 rounded-xl shadow-lg border border-white/10">
            <div className="flex justify-center mb-4">
              <AlertCircle className="w-12 h-12 text-red-400" />
            </div>
            <p className="text-white mb-4 font-medium text-lg">{error}</p>
            <Button
          ...
          <nav className="-mb-px flex space-x-8 overflow-x-auto">
            ...
            >
              <div className="flex items-center gap-2">
                <Timer className="w-4 h-4" /> Pendientes{' '}
                {pendientesCount > 0 && (
            ...
            >
              <div className="flex items-center gap-2">
                <Plane className="w-4 h-4" /> Próximos vuelos{' '}
                {proximosCount > 0 && (
                  <Badge className="bg-blue-600 text-white border-none shadow-lg shadow-blue-600/20">
                    {proximosCount}
                  </Badge>
                )}
              </div>
            ...
            >
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4" /> Vuelos pasados{' '}
                {pasadosCount > 0 && (
        ...
        <AnimatePresence mode="wait">
          {filteredReservations.length === 0 ? (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="text-center py-20 bg-white/5 backdrop-blur-md rounded-2xl border border-white/5"
            >
              ...
              <h3 className="text-xl font-semibold text-white mb-2">
                {activeTab === 'pendientes' ? '¡Todo al día!' : 'No se encontraron reservas'}
              </h3>
              <p className="text-slate-400 mb-8 max-w-md mx-auto">
                Explora nuestros destinos y empieza a viajar.
              </p>
              <Button
                onClick={() => navigate('/')}
                className="bg-blue-600 hover:bg-blue-500 text-white font-semibold px-8 py-3 h-auto rounded-full shadow-lg"
              >
                Explorar Vuelos
              </Button>
            </motion.div>
            ...
            >
              {filteredReservations.map((reservation) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className={`overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-blue-900/20 group bg-white/5 backdrop-blur-md border border-white/10 text-white ${
                        activeTab === 'pendientes' ? 'border-amber-500/30' : ''
                      }`}
                    >
                      {activeTab === 'pendientes' && (
                        <div
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <Timer
                              className={`w-5 h-5 ${
                                estaExpirando ? 'text-red-400' : 'text-amber-400'
                              }`}
                            />
                            <span
                              className={`font-semibold ${
                                estaExpirando ? 'text-red-300' : 'text-amber-300'
                              }`}
                            >
                              {tiempoRestante > 0
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-6">
                          <div className="flex-1">
                            <Badge
                              className={`${statusStyles[reservation.estado]} font-bold tracking-wide`}
                            >
                              {statusLabels[reservation.estado]}
                            </Badge>
                            <p className="text-xs text-slate-400 mt-2 font-mono">
                              REF: {reservation.codigoReserva}
                            </p>
                          </div>
                          {activeTab !== 'pendientes' && (
                            <Button
                          ...
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex-shrink-0 mx-4 flex flex-col items-center justify-center pt-2">
                              ...
                              <p className="text-[10px] text-blue-300/70 mt-1">
                                {reservation.vuelo.duracion || 'Directo'}
                              </p>
                            </div>
                            <div className="flex-1 text-right">
                              ...
                              <p className="text-sm text-slate-400 truncate max-w-[120px] ml-auto">
                        ...
                        {/* ✅ EQUIPAJE (Consistente con los demás) */}
                            ...
                            <div className="text-xs text-blue-100 flex flex-wrap gap-x-4 gap-y-1">
                              {reservation.equipaje.mano?.incluido ? (
                                <span className="flex items-center gap-1">
                                  ...
                                  <span className="opacity-70">
                                    ({reservation.equipaje.mano.peso}kg)
                                  </span>
                                </span>
                              ) : (
                                <span className="flex items-center gap-1 opacity-50">
                                  <X className="w-3 h-3" /> Sin Mano
                                </span>
                              )}
                              {reservation.equipaje.documentado?.incluido ? (
                                <span className="flex items-center gap-1">
                                  ...
                                  <span className="opacity-70">
                                    (
                                    {reservation.equipaje.documentado.piezasIncluidas +
                                      reservation.equipaje.documentado.piezasAdicionales}{' '}
                                    pza)
                                  </span>
                                </span>
                              ) : (
                                <span className="flex items-center gap-1 opacity-50">
                                  <X className="w-3 h-3" /> Sin Documentar
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="flex gap-3">
                          {activeTab === 'pendientes' && tiempoRestante > 0 ? (
                            <>
                              <Button
                                className="flex-1 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white shadow-lg shadow-blue-900/30 border-0"
                                onClick={() =>
                                  handleGoToPayment(reservation._id, reservation.precioTotal)
                                }
                              >
